...
public:
	...
	void detect(cv::Mat srcimg, std::vector<Object> &boxes,std::string file, bool photo = true);   ////只返回检测框,置信度和5个关键点这两个信息在后续的模块里没有用到
	...
	std::vector<Object> postprocessDetect_gpu(float* pData, int num_box, int channels,  std::string file);
